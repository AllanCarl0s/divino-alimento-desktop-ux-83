...
import { ArrowLeft, Search, Save, X, AlertCircle } from "lucide-react";
...
export default function AdminMercadoPrecosDetalhes() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  
  const mercado = mercadosLocais.find(m => m.id === id);
  
  // Verificar se o mercado pertence ao administrador atual
  const hasAccess = mercado?.administrador === CURRENT_ADMIN_NAME;
        ...
        <div className="flex items-center justify-center min-h-[400px]">
          <Card>
            <CardContent className="pt-6 text-center space-y-4">
              ...
              <Button onClick={() => navigate("/adminmercado/precos")}>
                Voltar para Gestão de Preços
              </Button>
            </CardContent>
          </Card>
        </div>
  ...
  // Caso não tenha acesso ao mercado
  if (!hasAccess) {
    return (
      <ResponsiveLayout>
        <div className="flex items-center justify-center min-h-[400px]">
          <Card className="max-w-md">
            <CardContent className="pt-6 text-center space-y-4">
              ...
              <p className="text-muted-foreground">Você não tem permissão para acessar este mercado.</p>
              <Button onClick={() => navigate("/adminmercado/precos")}>
                Voltar para Gestão de Preços
              </Button>
            </CardContent>
        ...
        <div className="flex flex-col">
          <div>
            ...
            <p className="text-muted-foreground">
              Defina preços específicos para este mercado com base nos produtos cadastrados
            </p>
          </div>
        </div>
