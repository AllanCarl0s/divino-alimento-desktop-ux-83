...
const AdminMercadoPagamentosGerir = () => {
  const navigate = useNavigate();
  const [pagamentos, setPagamentos] = useState<Pagamento[]>([]);
  const [busca, setBusca] = useState("");
  const [filtroTipo, setFiltroTipo] = useState<"Todos" | "Fornecedor" | "Consumidor">("Todos");
  ...
  useEffect(() => {
    ...
    const dadosSalvos = localStorage.getItem("pagamentosGeradosAdminMercado");
    if (dadosSalvos) {
      ...
      <div className="container max-w-6xl mx-auto py-8 px-4">
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-primary mb-2">
            Administrador de mercado - Gerir Lista de Pagamentos
          </h1>
          <p className="text-muted-foreground">
            Acompanhe, edite e registre os pagamentos pendentes e realizados apenas do seu mercado.
          </p>
        </div>
...
export default AdminMercadoPagamentosGerir;
